...
PitchDetector::PitchDetector(int sample_rate, int frame_size, float threshold)
...
PitchDetector::Result PitchDetector::process(const float* samples, int num_samples) {
    ...
    for (int i = 0; i < frame_size_; ++i) {
        frame[i] = ring_buffer_[(start + i) % (frame_size_ * 2)];
    }

    // Run YIN detection
    float freq = yin_->detect(frame.data());
...
int PitchDetector::frequency_to_midi(float frequency) {
    if (frequency <= 0.0f) return 0;
    float midi = 12.0f * std::log2(frequency / kA4_Hz) + static_cast<float>(kA4_Midi);
    return std::clamp(static_cast<int>(std::round(midi)), 0, 127);
}

float PitchDetector::midi_to_frequency(int midi_note) {
    return kA4_Hz * std::pow(2.0f, (static_cast<float>(midi_note - kA4_Midi)) / 12.0f);
}
